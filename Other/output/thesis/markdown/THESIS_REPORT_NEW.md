...
**Repository note:** For **current row counts, HRAST per-stem metrics, regenerated charts, and file paths** tied to the capstone `data/` and `output/` tree, see [`THESIS_IMPLEMENTATION.md`](THESIS_IMPLEMENTATION.md) **Section IX** and the rollups under `output/dataset analysis/`, `output/metrics/`, and `output/charts/`. This file remains **PDF-aligned verbatim** reference for the original thesis document.
