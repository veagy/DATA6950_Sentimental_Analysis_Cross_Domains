...
class MinMaxScaler(MLModule):
    ...
    def fit(self, data_or_X, y=None, **kwargs):
        ...
        # Match sklearn: constant features get scale=1, min_=feature_range[0]
        safe_range = torch.where(
            self.data_range_ > 0,
            self.data_range_,
            torch.ones_like(self.data_range_, device=self.data_range_.device, dtype=self.data_range_.dtype),
        )
...
class Normalizer(MLModule):
    ...
    def transform(self, X: torch.Tensor, **kwargs):
        ...
        # Avoid division by zero for zero-norm samples (match sklearn behavior)
...
class StandardScaler(MLModule):
    ...
    def fit(self, data_or_X, y=None, **kwargs):
        ...
        if self.with_std:
            # sklearn uses biased variance (ddof=0) for scale_
...
class RobustScaler(MLModule):
    ...
    def fit(self, data_or_X, y=None, **kwargs):
        ...
        # Convert percentiles to fractions for torch.quantile
        q = torch.tensor([q_min / 100.0, 0.5, q_max / 100.0], device=X.device)
        vals = torch.quantile(X, q, dim=0)  # Returns (3, n_features)

        # Center is the median (index 1 of the q tensor)
        if self.with_centering:
            self.center_ = vals[1]
        else:
            self.center_ = torch.zeros(self.n_features_in_, device=X.device)

        # Scale is the IQR (difference between 75th and 25th)
        if self.with_scaling:
            ...
            if self.unit_variance:
                # Adjust IQR to match unit variance of a Normal Distribution
                # The IQR of a standard normal distribution is ~1.34898
...
class PowerTransformer(MLModule):
    ...
    def _yeo_johnson_transform(self, x, lmbda):
        """Mathematically applies the Yeo-Johnson transformation."""
        ...
        # Case 1: x >= 0
        if torch.any(pos):
            if torch.abs(lmbda) < 1e-6:  # approx 0
        ...
        # Case 2: x < 0
        if torch.any(neg):
            if torch.abs(lmbda - 2) < 1e-6:  # approx 2
    ...
    def _log_likelihood(self, lmbda, x):
        """Log-likelihood function to minimize (negative for optimizer)."""
        n_samples = x.shape[0]
        ...
        # Log-likelihood formula for Yeo-Johnson
        log_like = -n_samples / 2 * torch.log(var)
        log_like += (lmbda - 1) * torch.sum(torch.sign(x) * torch.log1p(torch.abs(x)))
        return -log_like  # We want to maximize, so we minimize the negative

    def fit(self, data_or_X, y=None, **kwargs):
        """Estimates optimal lambda per feature using MLE."""
        X = torch.as_tensor(data_or_X, dtype=self.dtype, device=self.device)
        ...
        # Optimize lambda for each feature individually
        for i in range(n_features):
            feat = X[:, i]
            # Initialize lambda at 1.0 (no transform)
        ...
        # Final pass to calculate mean/std for standardization
        if self.standardize:
    ...
    def inverse_transform(self, data_or_X, **kwargs):
        """Apply inverse Yeo-Johnson transform using fitted lambdas."""
        X = torch.as_tensor(data_or_X, dtype=self.dtype, device=self.device)
...
class QuantileTransformer(MLModule):
    ...
    def _get_quantiles(self, X):
        """Helper to compute quantiles for each feature."""
        n_samples, n_features = X.shape
        n_quantiles = min(self.n_quantiles, n_samples)

        # Create reference points (0 to 1)
        references = torch.linspace(0, 1, n_quantiles, device=X.device)

        # Calculate quantile values for each feature
    ...
    def fit(self, data_or_X, y=None, **kwargs):
        """
        Estimate the CDF by computing the quantiles.
        """
        X = torch.as_tensor(data_or_X, dtype=self.dtype, device=self.device)
        self.n_features_in_ = X.shape[1]
        # Subsampling for efficiency if data is too large
    ...
    def transform(self, data_or_X, **kwargs):
        ...
        for i in range(n_features):
            # 1. Map original data to Uniform [0, 1] using interpolation
            # We treat 'quantiles_' as the x-axis and 'references_' as the y-axis
            feature_column = X[:, i]

            # Torch-only linear interpolation (per feature)
            interp_unit = self._interpolate(
                feature_column,
                self.quantiles_[:, i],
                self.references_
            )

            # 2. Map to Normal if requested
            if self.output_distribution == 'normal':
                # Map [0, 1] to [-inf, inf] using the probit function (erfinv)
                # We clip to avoid infinite values at the edges
                eps = 1e-7
    ...
    def inverse_transform(self, data_or_X, **kwargs):
        ...
        for i in range(n_features):
            feature_column = X[:, i]
            if self.output_distribution == "normal":
                # Map from normal back to uniform [0, 1] via CDF
                uniform = 0.5 * (1.0 + torch.erf(feature_column / torch.sqrt(torch.tensor(2.0, device=X.device, dtype=X.dtype))))
            else:
                uniform = feature_column
            # Map uniform back to original via inverse interpolation (references -> quantiles)
            X_orig[:, i] = self._interpolate(
                uniform,
                self.references_.flatten(),
                self.quantiles_[:, i].flatten(),
            )
        return X_orig

    def _interpolate(self, x, x_points, y_points):
        """Standard linear interpolation: maps x to y based on x_points/y_points (torch-only)."""
        x = x.flatten()
        x_points = x_points.flatten()
        y_points = y_points.flatten()

        # right_idx: index of first element in x_points strictly greater than x
        ...
        # Linear interpolation; avoid div by zero when x_left == x_right
        diff = x_right - x_left
